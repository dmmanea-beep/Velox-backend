...
// ─── DATA LOADING (Free Tier Optimized) ────────────────────────────────────
async function loadData() {
  ...
  // Processing operators one-by-one to avoid exceeding 512MB RAM
  for (const src of XML_SOURCES) {
    try {
      ...
      r.data = null; // Clear memory
    } catch (e) { console.error(`Failed: ${src.operator}`); }
  }
  dataReady = true;
  loadStatus = 'ready';
